...
export const BlogList: React.FC<BlogListProps> = ({ blogs, isLoading }) => {
  if (isLoading) {
    return (
        ...
        <div className="bg-zinc-900/30 backdrop-blur-sm border border-zinc-800/50 rounded-2xl p-6">
          ...
          <div className="space-y-1">
            {[...Array(8)].map((_, index) => (
              <div
                key={index}
                className="flex items-center justify-between py-4 px-4 rounded-xl animate-pulse"
              >
                {/* Left side */}
      ...
      <div className="relative">
        ...
        <div className="bg-zinc-900/30 backdrop-blur-sm border border-zinc-800/50 rounded-2xl p-6">
          <div className="text-center py-12">
            ...
            <p className="text-zinc-500 text-sm mt-2">Try adjusting your filters or check back later</p>
          </div>
        </div>
      </div>
    );
