...
interface BlogFiltersInteractiveProps {
  ...
  categories: Category[];
  tags: Tag[];
  onFilteredBlogsChange: (blogs: BlogWithDetails[]) => void;
}
...
export const BlogFiltersInteractive: React.FC<BlogFiltersInteractiveProps> = ({
  ...
  const filteredBlogs = useMemo(() => {
    let filtered: BlogWithDetails[] = [];

    if (activeFilter === "All posts") {
      filtered = initialBlogs.filter((blog) => blog.status === 'published' && blog.publishedAt);
      ...
      filtered = initialBlogs.filter((blog) => {
        ...
        return blog.tags.some((tag) => {
          ...
          // If it's just an ID, find it in tags array
          const tagObj = tags.find((t) => t._id === tag);
        ...
        <div className="mt-6 pt-6 border-t border-zinc-800/50">
          <div className="flex items-center justify-between text-sm">
            <div className="flex items-center gap-2 text-zinc-400">
              ...
              <span>
                Showing {activeFilter === "All posts" ? "all" : activeFilter}{" "}
                articles
              </span>
            </div>
