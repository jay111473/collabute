 ...
 */
export const ProjectManagerGrid: FC<ProjectManagerGridProps> = ({ projectManagers, isLoading, variant = 'compact' }) => {
  if (isLoading) {
  ...
  if (projectManagers.length === 0) {
    return (
      <div className="col-span-full text-center py-10">
        <p className="text-gray-400">No project managers found matching your criteria.</p>
      </div>
    );
  }
  ...
  return (
    <div className={`grid ${gridCols} gap-6`}>
      {projectManagers.map((projectManager) => (
        <ProjectManagerCard key={projectManager.id} projectManager={projectManager} variant={variant} />
      ))}
    </div>
  );
}; 